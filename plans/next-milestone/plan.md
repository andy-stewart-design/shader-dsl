...
Status: PR 1 and PR 2 merged; PR 3 Phase 3.3 decision recorded, pending PR review. This follows the [POC outcome](../_completed/poc/outcome.md); it is not a claim that the POC editor implementation is production-ready.
...
**Phase 3.3 result — integration decision recorded:** [The decision and editor matrix](./editor-integration-decision.md) retain the working VS Code provider and the private Zed LSP spike on the **same adapter**, without converting VS Code to LSP or claiming installed Zed semantics. The Zed highlighting and fixture-only diagnostic/hover feasibility goals are met; installable general Zed semantic support is **not met**. A root Zed worktree starts no Shdr server, while a separately opened `apps/editor-fixture` worktree does. The decision lists project discovery, scheduling, distribution and editor-quality requirements for broader support. No runtime changes were made in this phase.

**PR gate — verified, pending review:** VS Code highlighting works without regressing existing provider tests; Zed highlighting and fixture-only LSP diagnostic/hover have prior real Zed 1.21.0 screenshots and a reproducible checklist; the next decision is explicit above. At this checkpoint `pnpm build`, `pnpm check`, `pnpm test`, `pnpm ci:check`, `pnpm --filter @shdr/editor-fixture test:editor`, and `git diff --check` passed. Zed was not re-tested during the documentation-only Phase 3.3; the Phase 3.2 screenshots are its real-editor evidence. Full production LSP, semantic tokens, completion, rename, and multi-project support are not required to merge this PR.
