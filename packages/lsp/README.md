...
**Boundaries for the spike:** Only `file:` URIs ending in `.shdr.ts` are checked. Position conversion uses UTF-16 offsets, matching both TypeScript and LSP. The first initialized workspace folder (or `rootUri`) and its `tsconfig.json` are used for the whole process; project/config changes, multiple roots, project references and non-file URIs are not supported. Checking is synchronous on the LSP event loop, so cancellation cannot interrupt a check already running. Document versions prevent stale **incoming** edits from replacing newer state, but there is no concurrent-check scheduling. A real Zed 1.21.0 session verified one Shdr diagnostic and a mapped hover within the editor-fixture workspace. Opening the repository root with `zed -n .` does **not** start this server: the dev launcher accepts only `apps/editor-fixture` as its worktree. [Phase 3.3's decision](../../plans/next-milestone/editor-integration-decision.md) keeps the existing VS Code provider alongside this Zed LSP transport, both using the same adapter. Do not treat this as a production server or claim general project support, packaging, cancellation or config refresh.
